...
// util.inherits replacement using native Object.create.
...
// Minimal util.format replacement — handles %j and %s, good enough for avsc's error messages.
function f(fmt) {
  var i = 1;
  var args = arguments;
  return fmt.replace(/%[js]/g, function (m) {
    var v = args[i++];
    return m === '%j' ? JSON.stringify(v) : String(v);
  });
}

// util.deprecate replacement — wraps fn, warns once.
