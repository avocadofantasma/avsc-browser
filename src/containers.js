...
utils.inherits(RawDecoder, stream.Duplex);
...
utils.inherits(BlockDecoder, stream.Duplex);
...
utils.inherits(RawEncoder, stream.Transform);
...
utils.inherits(BlockEncoder, stream.Duplex);
...
BlockEncoder.prototype._read = function () {
  ...
  if (!data) {
    if (this._finished && !this._pending) {
      queueMicrotask(function () { self.push(null); });
    } else {
      this._needPush = true;
    }
    return;
...
function createBlobDecoder(blob, opts) {
  ...
  reader.onload = function (e) {
    rs.push(Buffer.from(e.target.result));
    rs.push(null);
  };
  reader.onerror = function (e) { rs.destroy(e.target.error); };
  reader.readAsArrayBuffer(blob);
  return rs.pipe(new streams.BlockDecoder(opts));
}

/** Write Avro values into a container and resolve with a Blob. */
function createBlobEncoder(schema, opts) {
  var encoder = new streams.BlockEncoder(schema, opts);
  var chunks = [];
