...
Message.prototype.isOneWay = utils.deprecate(
...
Service.prototype.createEmitter = utils.deprecate(
...
Service.prototype.createListener = utils.deprecate(
  function (transport, opts) {
...
Service.prototype.emit = utils.deprecate(
...
Service.prototype.equals = utils.deprecate(
  function (any) {
    return (
...
Service.prototype.getFingerprint = utils.deprecate(
  function (algorithm) {
...
Service.prototype.getSchema = utils.deprecate(
  Type.prototype.getSchema,
  'use `.protocol` instead of `.getSchema()`'
);

Service.prototype.on = utils.deprecate(
...
Service.prototype.subprotocol = utils.deprecate(
  function () {
...
utils.inherits(ClientChannel, events.EventEmitter);
...
ClientChannel.prototype.getCache = utils.deprecate(
  function () { return this.client._cache$; },
  'use `.remoteProtocols()` instead of `.getCache()`'
);

ClientChannel.prototype.getProtocol = utils.deprecate(
  function () {
...
ClientChannel.prototype.isDestroyed = utils.deprecate(
...
utils.inherits(StatelessClientChannel, ClientChannel);
    ...
    var writable = self._writableFactory.call(self, function (err, readable) {
      ...
      readable.on('data', function (obj) {
        ...
        if (match === 'NONE') {
          ...
          queueMicrotask(function() { emit(true); });
        } else {
...
function StatefulClientChannel(client, readable, writable, opts) {
  ...
  function onPing(obj) {
    ...
    self.emit('handshake', hreq, hres);
    if (match === 'NONE') {
      queueMicrotask(function () { ping(true); });
    } else {
...
utils.inherits(StatefulClientChannel, ClientChannel);
...
utils.inherits(ServerChannel, events.EventEmitter);
...
ServerChannel.prototype.getCache = utils.deprecate(
  function () { return this.server._cache; },
  'use `.remoteProtocols()` instead of `.getCache()`'
);

ServerChannel.prototype.getProtocol = utils.deprecate(
  function () {
...
ServerChannel.prototype.isDestroyed = utils.deprecate(
  function () { return this.destroyed; },
  'use `.destroyed` instead of `.isDestroyed`'
);
...
utils.inherits(StatelessServerChannel, ServerChannel);
...
utils.inherits(StatefulServerChannel, ServerChannel);
...
utils.inherits(FrameDecoder, stream.Transform);
...
utils.inherits(FrameEncoder, stream.Transform);
...
utils.inherits(NettyDecoder, stream.Transform);
...
utils.inherits(NettyEncoder, stream.Transform);
